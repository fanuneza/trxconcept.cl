...
**Add a new WhatsApp CTA** — always set both `data-wa` and a specific `data-wa-msg` (see "WhatsApp link mechanism" in `AGENTS.md`/`spec.md`). Don't rely on the generic fallback for a primary CTA; the whole point of `data-wa-msg` is that every button says what happens next.
