...
- `public/assets/img/*.avif`/`*.webp` are legacy static variants kept for backward compatibility; current pages import images from `src/assets/images/` and process them through `astro:assets`, including the JSON-LD structured data in `src/data/pages.ts`.

## WhatsApp number and message construction

- The WhatsApp number and default message live in `src/data/site.ts` (`WHATSAPP_NUMBER`, `WHATSAPP_DEFAULT_MESSAGE`, and the derived `whatsappHref`). `public/assets/js/main.js` reads the default message from the `<html data-wa-default-msg>` attribute set in `BaseLayout.astro`, so the client-side rewrite stays in sync with the server-side source of truth. Only `WA_NUMBER` remains hardcoded in `main.js` as a defensive fallback.
