...
pages.ts (per-page `structuredData`, `breadcrumb`, `title`, `description`, `canonical`, `isHome`)
   ...
   │      + optional `head` slot for homepage-only additions (e.g., the `WebSite` site-name schema)
...
| Shared JSON-LD graph shape (Organization/WebPage/Breadcrumb; WebSite is homepage-only) | `src/utils/schema.ts` + `src/pages/index.astro` head slot                                                                                                                                                     |
