...
for (const v of variants) {
  ...
  await page.evaluate(async () => {
    ...
    await Promise.all(
      [...document.images].map((i) => (i.complete ? Promise.resolve() : i.decode().catch(() => {}))),
    );
  });
  ...
  await sharp(pngBuf).resize(1200, 630).png().toFile(join(DIR, `og-${v}.png`));
  await sharp(pngBuf).resize(1200, 630).webp({ quality: 82 }).toFile(join(DIR, `og-${v}.webp`));
  const { size } = await sharp(join(DIR, `og-${v}.webp`)).metadata().then(() => import("node:fs")).then((fs) => fs.statSync(join(DIR, `og-${v}.webp`)));
  console.log(`${v}: og-${v}.webp ${Math.round(size / 1024)}KB`);
}

// Contact sheet: 5 stacked at 600x315 with 16px gaps on a dark board.
const thumbW = 600, thumbH = 315, gap = 16;
const thumbs = await Promise.all(variants.map((v) => sharp(join(DIR, `og-${v}.png`)).resize(thumbW, thumbH).png().toBuffer()));
const board = sharp({
  create: {
    width: thumbW + gap * 2,
